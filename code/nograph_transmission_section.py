...
class TransmissionSectionEnv(gym.Env):
    ...
    def __init__(self, env_name, evaluation=False):
        # 电网环境名
        if env_name == 'S4case118' or env_name == 'S10case118':
            self.original_net = pn.case118()           # 初始电网样本
        elif env_name == 'S4case300' or env_name == 'S10case300':
        ...
        if not os.path.exists(os.path.join(ROOT_PATH, env_name, 'multi_train_control_nets.pt')):
            assert False, 'check data'

        if not self.evaluation:
            self.control_nets = load_variable(os.path.join(ROOT_PATH, env_name, 'multi_train_control_nets.pt'))
        else:
            self.control_nets = load_variable(os.path.join(ROOT_PATH, env_name, 'multi_test_control_nets.pt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_id', type=str, default='S4case118', help="电网环境名") # S4:4个断面中随机选1个调整，S10:10个断面中随机选一个调整，S4更简单
    args = parser.parse_args()

    env = TransmissionSectionEnv(args.env_id, evaluation=True)
    print(env.action_space.n)  # 动作空间，53台发电机，每台发电机可上下调整20%，共106个离散动作空间
    print(env.observation_space.shape[0])  # 观测空间，共118个母线，每个母线有p q v theta 4个特征，加上173条输电线的onehot向量表示当前调整断面的组成，共118*4+173=645维
    print(env.n_line)  # 上述输电线的数量
    obs = env.reset()
    print(len(obs))
    print(type(obs))
    ...
    # for i in range(10):
    #     env.reset()  # 随机reset10个状态
    #     done = False
    #     step = 0
    #     t_reward = 0
    #     while not done:
    #         act = np.random.randint(n_action)
    #         obs, reward, done, info = env.step(act)
    #         step += 1
    #         t_reward += reward
    #     print(env.current_idx, step, t_reward)
